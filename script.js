...
function initSiteAudio(){
	...
	try{
		...
		if(ds.title && titleEl) titleEl.textContent = ds.title;
		if(ds.artist && artistEl) artistEl.textContent = ds.artist;
		if(ds.art && artImg){ artImg.src = ds.art; artImg.alt = ds.title || artImg.alt; }
		
		// Make album art clickable to open preview
		if(artImg){ artImg.style.cursor = 'zoom-in'; artImg.addEventListener('click', ()=> openAlbumPreview(ds.art || artImg.src, ds.title || artImg.alt)); }
	}catch(e){ /* ignore */ }
